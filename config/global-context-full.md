...
6. **NO build steps**: Do NOT use build tools (Vite, Webpack, etc.) or `.vue`/`.jsx` files. Write plain JavaScript that runs directly in the browser. Download pre-built libraries once.
...
✅ GOOD (no build needed):
- Plain .js files with ES modules
- Pre-built library files (.min.js)
- Direct browser JavaScript
```

### 📦 Vue/React WITHOUT Build

**If using Vue, write components in plain .js files:**

```
project/
├── libs/                    # Download once, never rebuild
│   ├── vue.global.min.js
│   ├── vue-router.global.min.js
│   └── pinia.global.min.js
├── js/
│   ├── components/
│   │   └── Header.js        # Plain JS, not .vue!
│   ├── stores/
│   │   └── auth.js
│   └── app.js
└── index.html
...
<script src="libs/vue-router.global.min.js"></script>
<script type="module" src="js/app.js"></script>
```

### 📥 Download Libraries (NO CDN!)

**Download pre-built .min.js files to libs/ folder:**

```bash
# Download Vue ecosystem
mkdir -p libs
curl -o libs/vue.global.min.js https://unpkg.com/vue@3/dist/vue.global.prod.js
curl -o libs/vue-router.global.min.js https://unpkg.com/vue-router@4/dist/vue-router.global.prod.js
curl -o libs/pinia.global.min.js https://unpkg.com/pinia@2/dist/pinia.iife.prod.js

# Or use npm just to download, then copy dist files
npm install vue && cp node_modules/vue/dist/vue.global.prod.js libs/
```

**Why no CDN links in HTML:**
- Works offline
- Faster (no external requests)
- More secure
- Reliable

**Exceptions (OK to use CDN):**
- Google Maps API
- Google Fonts
- APIs that require remote loading

### 🎨 UI Component Libraries (Recommended)

**⚠️ ASK USER FIRST:** If the project needs grids/calendars/rich UI, ask the user if they have their own library (e.g., Kendo UI, DevExtreme). If they provide one, use that.

**Default (free) options if user has no preference:**

| Need | Library | Notes |
|------|---------|-------|
| **All-in-one** | **PrimeVue** ⭐ | Grid, Calendar, Forms, Charts - works with Vue global build |
| **Data Grid** | AG Grid Community | Best grid, free community edition |
| **Calendar** | FullCalendar | Best calendar, vanilla JS |
| **Simple Grid** | Tabulator | Lightweight, 100% free |
| **jQuery Grid** | DataTables | Classic, stable |

**PrimeVue Setup (Recommended):**
```bash
# Download PrimeVue for global/no-build use
mkdir -p libs/primevue
curl -o libs/primevue/primevue.min.js https://unpkg.com/primevue/umd/primevue.min.js
curl -o libs/primevue/datatable.min.js https://unpkg.com/primevue/datatable/datatable.min.js
curl -o libs/primevue/calendar.min.js https://unpkg.com/primevue/calendar/calendar.min.js
curl -o libs/primevue/theme.css https://unpkg.com/primevue/resources/themes/lara-light-blue/theme.css
```

```html
<!-- index.html -->
<link href="libs/primevue/theme.css" rel="stylesheet">
<script src="libs/vue.global.min.js"></script>
<script src="libs/primevue/primevue.min.js"></script>
<script src="libs/primevue/datatable.min.js"></script>
...
**AG Grid Setup (Best standalone grid):**
```bash
curl -o libs/ag-grid-community.min.js https://unpkg.com/ag-grid-community/dist/ag-grid-community.min.js
```
